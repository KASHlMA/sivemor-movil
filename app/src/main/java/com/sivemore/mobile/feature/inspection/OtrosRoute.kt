...
private fun OtrosContent(
    ...
    LazyColumn(
        ...
        item {
            Text(
                text = state.errorMessage ?: state.otrosSection.description,
        ...
        items(state.otrosSection.questions, key = { it.id }) { question ->
            OtrosInspectionCard(
                question = question,
                onOptionSelected = { optionId ->
                    onAction(InspectionFlowAction.OtrosOptionSelected(question.id, optionId))
                },
            )
        }
        item {
...
@Composable
private fun OtrosInspectionCard(
    question: InspectionQuestionItem,
    onOptionSelected: (String) -> Unit,
    modifier: Modifier = Modifier,
) {
    VerificationCard(modifier = modifier.testTag("otros_card_${question.id}")) {
        Text(
            text = question.title,
            ...
            question.options.forEach { option ->
                Row(
                    modifier = Modifier
                        ...
                        .testTag("otros_option_${question.id}_${option.id}"),
...
@PhonePreview
@Composable
private fun OtrosScreenPreview() {
    SivemoreTheme {
        OtrosScreen(
            state = InspectionFlowUiState(
                vehicleId = "1",
